...
class OPVHGraph(InMemoryDataset):
    ...
    def __init__(self, root, polymer=False, partition='train',
                 transform=None, pre_transform=None, pre_filter=None,  dsize = None, delocalization = None):
    ...
    def compute_hgraph_data(self, df, dsize = None):
        # create hgraph data list from pd.DataFrame object
        if dsize == None:
            smiles = df['smile'].values.tolist()
            target = df.iloc[:, 2:].values
        else:
            smiles = df['smile'].values.tolist()[:dsize]
            target = df.iloc[:, 2:].values[:dsize]
    ...
    def process(self):
        ...
        data_list = self.compute_hgraph_data(df, dsize = self.dsize)
