...
if __name__ == '__main__':
    ...
    parser.add_argument('--delocalization', action='store_true', default=False, help='includes the delocalizaiton hyperedges')
    parser.add_argument('--dsize', type=int, default=1000, help='training data size')
    ...
    if args.method == 'mhnn':
        train_dataset = OPVHGraph(root=args.data_dir, polymer=args.polymer, partition='train', transform=transform, delocalization=args.delocalization, dsize = args.dsize)
        valid_dataset = OPVHGraph(root=args.data_dir, polymer=args.polymer, partition='valid', transform=transform, delocalization=args.delocalization)
        test_dataset = OPVHGraph(root=args.data_dir, polymer=args.polymer, partition='test', transform=transform, delocalization=args.delocalization)
    else:
